...
/**
 * POST /api/ai/simulation-review
 *
 * Reviews a completed exam simulation, including time analysis
 * and essay quality feedback.
 *
 * Body: { essay, subject, topic, stageData, timeAnalysis }
 * Returns: structured JSON with time analysis and essay feedback.
 */
export async function POST(req: NextRequest) {
  try {
    const body = await req.json()
    const { essay, subject, topic, stageData, timeAnalysis } = body

    // --- Validation ---
    if (!essay || essay.length < 10) {
      return NextResponse.json(
        { error: '作文内容太短，请写更多内容后再提交' },
        { status: 400 },
      )
    }
    ...
    const systemPrompt = `你是一位资深的${langLabel}写作教师和考试策略顾问。
请从作文质量和考试策略两个维度给出综合评价。
